...
ENV_DIR = os.path.join(ROOT_DIR, 'envs', 'flappy')
...
from arguments.flappy_neat import get_args


def main():
    args = get_args() # コマンドライン引数に関する処理

    save_path = os.path.join(CURR_DIR, 'out', 'flappy_neat', args.name) # 実験結果の出力先
    ...
    # 設定ファイルの読み込み(.cfg)
    config_file = os.path.join(CURR_DIR, 'config', 'flappy_neat.cfg')
    # 引数で渡された設定の上書き
    custom_config = [
        ('NEAT', 'pop_size', args.pop_size),
    ]
    config = neat_cppn.make_config(config_file, custom_config=custom_config)
    config_out_file = os.path.join(save_path, 'flappy_neat.cfg')
